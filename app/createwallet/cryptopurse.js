...
const CryptoPurseABI: ethers.ContractInterface = [
...
export const deposit = async (amount: string): Promise<void> => {
  try {
...
export const withdraw = async (amount: string): Promise<void> => {
  try {
    const contract = await getContract();
...
export const checkBalance = async (): Promise<string> => {
  try {
    const contract = await getContract();
    const balance: ethers.BigNumber = await contract.checkBalance();
